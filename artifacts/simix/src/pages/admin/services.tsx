      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-400 text-sm mt-1">
            {activeServices} actifs · {inactiveServices} inactifs · Gérez les prix par pays dans « Tarification »
          </p>
        </div>
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-2 text-xs text-zinc-500 bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2">
            ...
            <span>Prix personnalisés par pays → Tarification</span>
          </div>
          {tab === "services" && (
          ...
          {tab === "services" ? (
            services?.length === 0 && !loadingS ? (
              ...
              <table className="w-full">
                ...
                <tbody>
                  {loadingS
                    ? <tr><td colSpan={8} className="py-12 text-center"><Loader2 className="w-6 h-6 text-violet-500 animate-spin mx-auto" /></td></tr>
