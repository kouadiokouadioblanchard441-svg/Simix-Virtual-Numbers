...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    if (user.balance < price) {
      res
        .status(402)
        .json({ error: "Solde insuffisant. Rechargez votre portefeuille." });
      return;
    }
    ...
    if (!vn) {
      res.status(500).json({ error: "Erreur création du numéro" });
      return;
    }
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    if (!row) {
      res.status(404).json({ error: "Numéro introuvable" });
      return;
    }
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    if (user.balance < EXTEND_FEE) {
      res.status(402).json({ error: "Solde insuffisant pour prolonger" });
      return;
    }
    ...
    const newExpiresAt = new Date(
      Math.max(row.n.expiresAt.getTime(), Date.now()) +
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    // Refund only if no messages received
